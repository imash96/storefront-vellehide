...
import type { ClientFetch, Config, FetchArgs, FetchInput, FetchStreamResponse, Logger, } from "@medusajs/js-sdk"
...
const normalizeRequest = (
    ...
    const credentials = config.auth?.type === "session" ? config.auth?.fetchCredentials || "include" : "omit"
...
export class Client {
    ...
    private DEFAULT_JWT_STORAGE_KEY = "__auth_token"
    private token = ""
    ...
    async fetchStream(
        ...
        const res = await this.fetch_(input, {
            ...init,
    ...
    protected getApiKeyHeader_ = (): { Authorization: string } | object => {
        return this.config.apiKey ? { Authorization: "Basic " + toBase64(this.config.apiKey + ":") } : {}
    }

    protected getPublishableKeyHeader_ = (): | { [PUBLISHABLE_KEY_HEADER]: string } | object => {
        return this.config.publishableKey ? { [PUBLISHABLE_KEY_HEADER]: this.config.publishableKey } : {}
    }

    protected async getJwtHeader_(): Promise<{ Authorization: string } | object> {
