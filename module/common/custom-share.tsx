...
const sizeClasses = {
    ...
    md: 'w-10 h-10 sm:w-11 sm:h-11 text-base',
...
export function ShareButtons({
    ...
    const platformConfig = {
        ...
        email: {
            name: 'Email',
            color: 'bg-muted hover:bg-muted-hover text-text-primary',
            icon: (
    ...
    if (variant === 'compact') {
        return (
            <div className={`inline-flex items-center gap-2 ${className}`}>
                <span className="text-xs sm:text-sm text-text-secondary whitespace-nowrap">Share:</span>
                {platforms.map((platform) => (
                    <button
                        ...
                        className={`
                            ...
                            active:scale-95
                        `}
                        aria-label={`Share on ${platformConfig[platform].name}`}
                    >
        ...
        return (
            <div className={`flex flex-wrap gap-2 sm:gap-3 ${className}`}>
                {platforms.map((platform) => (
                    ...
                    >
                        {platformConfig[platform].icon}
                        <span>{platformConfig[platform].name}</span>
                    </button>
                ))}
            </div>
        );
